...
const providerEducation =db.providerEducation;
const providerExperience =db.providerExperience;
const userIdentity =db.userIdentity
...
module.exports = {
  ...
  tag_id:async function(){
      let tag = crypto.randomBytes(25).toString('hex');
      return tag;
  ...
  var   get_user_data = await user.findOne({
    ...
    get_user_data.dataValues.identity =await this.get_identity_user(get_user_data.dataValues.id);
  ...
  verify_email: async function (get_param, req, res, user_type) {
    ...
    if (data) {
      ...
      var otp =Math.floor(Math.random() * 10000);
        var mailOptions = {
          ...
          to: get_param.email,
          subject: 'Valeyou Verification Email',
      ...
      transporter.sendMail(mailOptions, function (error, info) {
        ...
        var update_otp = await provider.update({
          otp: otp,
        },
          {
            where: {
  ...
  check_email: async function(data,req,res){
  ...
  verify_email_otp : async function(data,req,res){
    var check_otp = await provider.findOne({
      where:{
        email:data.email
      },
      raw:true
  ...
  file_upload: async function (get_image_data) {
    ...
    if (Array.isArray(image) === true) {
      ...
      var fileimage = uuid() + extension;
      image.mv(process.cwd() + '/valeyou/public/upload/' + fileimage, function (err) {
        if (err)
          console.log(err);
      });
      return fileimage;
...
// [{"category_id" : "18","subcategory" : [{"id": "2","price":"20"},{"id": "3","price":"30"}]}]
  ...
  //  get_signup_data.dataValues.subCategories = get_provider_cate_subcat
    ...
    var tag_id =await this.tag_id()+'_'+create_signup_user.dataValues.id ;
    var update_tag = await provider.update({
      tagId: tag_id
    },{
      where:{
        id:create_signup_user.dataValues.id
  ...
  provider_login: async function (data, convert_data, get_auth_key, req, res) {
    ...
    check_email_password = await provider.findOne({
      attributes:
      ['id', 'firstName','email','description', 'lastName', 'image', 'countryCode', 'phone', 'driverLicence', 'socialSecurityNo', 'authKey', 'state', 'city', 'address','age','dob','paypalId','online','tagId'],
      where: {
        email: data.email,
        password: convert_data
      }
    ...
    if (check_email_password) {
      ...
      var tag_id = await this.tag_id()+'_'+check_email_password.dataValues.id ;
      //console.log(tag_id,"==============tag_id")
    var update_tag = await provider.update({
      tagId: tag_id
    },{
      where:{
        id:check_email_password.dataValues.id
      }
    })
  ...
  provider_social_login: async function (get_data_login) {
   ...
   var  check_social_id = await provider.findOne({
      where: {
    ...
    // console.log(check_social_id,"check_social_id");return;
     ...
     if (check_social_id) {
      ...
      var get_provider_profile ={}
      if(get_social_data){
        var data ={}
        data.provider_id=get_social_data.id
        var check_auth =get_social_data;
         get_provider_profile = await this.get_provider_profile(data, check_auth)
  ...
  get_provider_details: async function (get_data, check_auth) {
    try {
      ...
      get_provider_data = await database.query("SELECT id ,first_name,last_name,name,country_code,tag_id,phone,image,description,address,email, (select count(id) from favourite where user_id='" + check_auth.dataValues.id + "' and provider_id='" + get_data.provider_id + "') as like_status, (select round(ifnull(avg(ratings),0),1) from rating where userTo='" + get_data.provider_id + "' and type=1) as average_rating,(select count(id) from rating where userTo='" + get_data.provider_id + "' and type=1) as total_ratings ,(select count(id) from `order` where provider_id=14 and status=4) as totalprovidedservices FROM `provider` where id='" + get_data.provider_id + "' group by  id limit 1", {
      ...
      if(get_provider_data){
        ...
        final.ProviderReview = get_rating;
...
//console.log(JSON.stringify(get_data.selected_data),"===================get_data.selected_data")
  ...
  get_job_details: async function (get_data, check_auth) {
    try {
      ...
      get_job_data = await order.findOne({
        attributes: ['id', 'date','providerId','userId','title','startTime','endTime','startPrice','endPrice','state', 'description', 'location', 'status','date','time','type','jobType'],
        include: [
          {
            model: orderCategory,
            attributes:['id','categoryId'],
  ...
  filter_search: async function(requestdata){
    try{
      var my_query ='';
      my_query ='select id,first_name as firstName, last_name as lastName,description,image,latitude,longitude,address,profession,state,online,tag_id as tagId,round((3959 * acos (cos ( radians('+requestdata.latitude+') )* cos( radians( latitude ) )* cos( radians( longitude ) - radians('+requestdata.longitude+') )+ sin ( radians('+requestdata.latitude+') )* sin( radians( latitude ) ) )),2) AS distance ,(select ifnull(round(avg(ratings),1),0) as rat from rating where userTo = p.id and type =1) as rating,(select count(*)  as total from `order` where provider_id=p.id and status !=2) as total_jobs,(select count(id) as tt from favourite where provider_id = p.id and user_id ='+ requestdata.user_id +') as fav from provider as p where p.online =1';
      if(requestdata.search){
        my_query+=' and  first_name  like "%'+requestdata.search+'%" or last_name like  "%'+requestdata.search+'%" or tagId like  "%'+requestdata.search+'%"';
       }
  ...
  filter_search_provider: async function(requestdata){
    try{
      var my_query ='';
      my_query ='select id,user_id as userId, provider_id as providerId,description,title,latitude,longitude,location,state,status,date,round((3959 * acos (cos ( radians('+requestdata.latitude+') )* cos( radians( latitude ) )* cos( radians( longitude ) - radians('+requestdata.longitude+') )+ sin ( radians('+requestdata.latitude+') )* sin( radians( latitude ) ) )),2) AS distance ,ifnull((SELECT ifnull(price,0) as bid_price from bids where provider_id='+requestdata.user_id+' and order_id=p.id),0) as bid_price from `order` as p where p.status =0';
...
//  pagination---------------------
    ...
    condition.status = 0
    condition.id=order_ids

  //  console.log(condition,"====================order_ids");return;
      get_data_home_current = await order.findAll({
  ...
  get_provider_job_details: async function (get_data, check_auth) {
    try {
      ...
      get_provider_job_details_data = await order.findOne({
        attributes: ['id', 'date','title','startTime','endTime','startPrice','endPrice', 'description', 'location', 'status','date','time','jobType','type','startjobDate','endjobDate','state','city','zipCode','appartment','street',[sequelize.literal(`ifnull((SELECT price FROM bids WHERE order_id = order.id and provider_id=${check_auth.dataValues.id}),0)`), 'bid_price'],[sequelize.literal('(select count(id) as tt from fav_jobs where provider_id = '+ check_auth.dataValues.id +' and job_id=order.id)'),'fav']],
            include: [
              {
                model: orderCategory,
                attributes:['id','categoryId'],
  ...
  get_provider_profile: async function (get_data, check_auth) {
    try {
      ...
      get_profile_data.education =await this.get_education_provider(get_profile_data);
      get_profile_data.experience =await this.get_experience_provider(get_profile_data);
    ...
    try {
      ...
      // if (data.status == 6) {
        let get_cname = await order.findOne({
          attributes: ['id', 'userId','providerId','startjobDate','endjobDate'],
          where: {
            id: data.job_id
          },raw:true
        })
  ...
  get_identity_provider: async function(data){
    if(data){
      var get_data = await providerIdentity.findAll({
        attributes:['id','providerId','image','backImage'],
        where:
        {
          providerId:data
        },
    ...
    add_identity_user: async function(data){
      if(data){
        var insert_data = await userIdentity.create(data);
        if(insert_data){
          return insert_data;
        }
      }
    },
    edit_identity_user: async function(data){
      if(data){
        var update_data = await userIdentity.update(data,{where:{id:data.identity_id}});
        if(update_data){
          return update_data;
        }
      }
    },
    delete_identity_user: async function(data){
      if(data){
        var delete_data = await userIdentity.destroy({where:{id:data.identity_id}});
        if(delete_data){
          return delete_data;
        }
      }
    ...
    get_identity_user: async function(data){
      if(data){
        var get_data = await userIdentity.findAll({
          where:
          {
            userId:data
          },
  ...
  IosPushNotification: function (data) {
   ...
   /****************  education  ********************/ 
  add_education_provider: async function(data){
    if(data){
      var insert_data = await providerEducation.create(data);
      if(insert_data){
        return insert_data;
      }
    }
  },
  edit_education_provider: async function(data){
    if(data){
      var update_data = await providerEducation.update(data,{where:{id:data.id}});
      if(update_data){
        return update_data;
      }
    }
  },
  delete_education_provider: async function(data){
    if(data){
      var delete_data = await providerEducation.destroy({where:{id:data.id}});
      if(delete_data){
        return delete_data;
      }
    }
  ...
  get_education_provider: async function(data){
    if(data){
      var get_data = await providerEducation.findAll({where:{providerId:data.id},order:[['endDate','desc']],raw:true});
      if(get_data){
        return get_data;
      }
    }
  ...
  add_experience_provider: async function(data){
    if(data){
      var insert_data = await providerExperience.create(data);
      if(insert_data){
        return insert_data;
      }
    }
  },
  edit_experience_provider: async function(data){
    if(data){
      var update_data = await providerExperience.update(data,{where:{id:data.id}});
      if(update_data){
        return update_data;
      }
    }
  },
  delete_experience_provider: async function(data){
    if(data){
      var delete_data = await providerExperience.destroy({where:{id:data.id}});
      if(delete_data){
        return delete_data;
      }
    }
  ...
  get_experience_provider: async function(data){
    if(data){
      var get_data = await providerExperience.findAll({where:{providerId:data.id},order:[['currentlyWorking','desc']],raw:true});
      if(get_data){
        return get_data;
      }
    }
